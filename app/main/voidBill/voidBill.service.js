...
angular.module('itouch.services')
  .factory('VoidBillService', ['$q', '$localStorage', 'DB', 'DB_CONFIG', 'ItemService', 'BillService', 'ControlService', 'ShiftService', 'LogService',
    function ($q, $localStorage, DB, DB_CONFIG, ItemService, BillService, ControlService, ShiftService, LogService) {
      var self = this;
      ...
      self.voidBill = function (DocNo) {
        return $q.all({
          ...
          data.items = _.map(data.items, function (item) {
            ...
            eventLog.log('voidBill reverse : items');
          ...
          data.transactions = _.map(data.transactions, function (item) {
            item.IsExported = false;
            item.DocNo = data.header.DocNo;
            eventLog.log('voidBill reverse : transactions');
          ...
          data.overTender = _.map(data.overTender, function (item) {
            item.IsExported = false;
            item.DocNo = data.header.DocNo;
            eventLog.log('voidBill reverse : overTender');
            return reverse(item, ['Amount', 'ChangeAmount']);
          });
          ...
          data.discounts = _.map(data.discounts, function (item) {
            item.IsExported = false;
            item.DocNo = data.header.DocNo;
            eventLog.log('voidBill reverse : discounts');
          ...
          data.stockTransactions = _.map(data.stockTransactions, function (item) {
            ...
            item.DocNo = data.header.DocNo;
            item.DocType = 'VD';
            eventLog.log('voidBill reverse : stockTransactions');
